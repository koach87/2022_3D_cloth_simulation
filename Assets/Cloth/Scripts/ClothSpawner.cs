...
public class ClothSpawner : MonoBehaviour {
	...
	public int method = 0; //{EULARIAN, RK2, RK4};
	...
	public void firstRun()
    {
		...
		if (method == 0)
			...
			clothCompute.SetInt("euler", 2);
		else if (method == 2)
			clothCompute.SetInt("euler", 4);


		//else if (method == IntegrationMethod.LEAPFROG) {
    ...
    {
		//rend view of force or connection
		rendView();
	}
	...
	void OnDrawGizmos() {
		...
		//			Gizmos.DrawLine(vertices[i], vertices[i] + Vector3.Normalize(forces[i]));
		//			//OnPostRenderr(vertices[i], vertices[i] + Vector3.Normalize(forces[i]), Color.blue);
		//		}
		//	}
		...
		//			// _
		//			Gizmos.DrawLine(vertices[i * dim + j], vertices[i * dim + j - 1]);
		//			// \
		//			Gizmos.DrawLine(vertices[i * dim + j], vertices[(i - 1) * dim + j - 1]);
		//			// |
		//			Gizmos.DrawLine(vertices[i * dim + j], vertices[(i - 1) * dim + j]);
		//			// /
		//			Gizmos.DrawLine(vertices[i * dim + j], vertices[(i - 1) * dim + j + 1]);

		//			//// _
		//			//OnPostRenderr(vertices[i * dim + j], vertices[i * dim + j - 1], Color.yellow);
		//			//// \
		//			//OnPostRenderr(vertices[i * dim + j], vertices[(i - 1) * dim + j - 1], Color.yellow);
		//			//// |
		//			//OnPostRenderr(vertices[i * dim + j], vertices[(i - 1) * dim + j], Color.yellow);
		//			//// /
		//			//OnPostRenderr(vertices[i * dim + j], vertices[(i - 1) * dim + j + 1], Color.yellow);
    ...
    private void OnPostRenderr(Vector3 startVertex, Vector3 endVertex, Color c)
    {
		//if (!mat)
		//{
		...
		//}
		Debug.Log("draw");
        GetComponent<matForLine>().mat.SetPass(0);
        GL.Begin(GL.LINES);
        GL.Color(c);
